...
probe_ft_weights = "best_ft_probe.pth" 
probe_base_weights = "best_base_probe.pth" 
...
probe_ft = DenseLinearProbe(backbone_ft.config.hidden_size, num_classes).to(device)
probe_ft.load_state_dict(torch.load(probe_ft_weights, map_location=device))
probe_ft.eval()

probe_base = DenseLinearProbe(backbone_base.config.hidden_size, num_classes).to(device)
